...
public class TopLevelStarListEmpty extends SGTDBF<IConstructor, ITree, ISourceLocation> implements IParserTest{
	...
	public ITree executeParser(){
		return parse(LIST1, null, "".toCharArray(), new DefaultNodeFlattener<IConstructor, ITree, ISourceLocation>(), new UPTRNodeFactory(true));
	}
	
	public IValue getExpectedResult() throws IOException{
		String expectedInput = "appl(regular(iter(sort(\"A\"))),[])";
		return new StandardTextReader().read(ValueFactoryFactory.getValueFactory(), RascalValueFactory.uptr, RascalValueFactory.Tree, new StringReader(expectedInput));
	}
	
	public static void main(String[] args){
		TopLevelStarListEmpty nrpl = new TopLevelStarListEmpty();
