...
data Exp fun(Var name, Exp exp) | Var Name | apply(Exp exp1, Exp exp2);
...
public set[Var] boundVars(Exp E) {
    return {Name | fun(Var Name, Exp E1) : E};
} 
...
*/
      case fun(Var V2, Exp Body):
        if(V1 == V2)
        	insert fun(V2, subst(V1, V2, Body));
        	
        else if(V2 in freeVars(Repl)){
           Var V2f = fresh(V2, freeVar(Body) + freeVars(Repl));
           insert fun(V2f, subst(V1, Repl, subst(V2, V2f, Body)));        
        } else
        	insert fun(V2, Body);
    };
}
