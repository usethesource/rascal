...
map[tuple[Symbol symbol, map[Symbol,Production] definitions], map[Symbol,Production]] reifyCache = ();

private map[Symbol symbol, type[value] resType] symbolToValueCache = ();
...
private bool hasManualTag(\choice(Symbol def, set[Production] alternatives)) =
...
private rel[Symbol,Symbol] getDependencies(s1: Symbol::\label(_,Symbol s2)) = <s1, s2> + getDependencies(s2);
private rel[Symbol,Symbol] getDependencies(s1: Symbol::\set(Symbol s2)) = <s1, removeOuterLabel(s2)> + getDependencies(s2);
...
private rel[Symbol,Symbol] getDependencies(c:  Symbol::\cons(Symbol \adtsym, str name, list[Symbol] parameters)) =
	{<c, removeOuterLabel(sym)> | Symbol sym <- parameters} + { *getDependencies(sym) | Symbol sym <- parameters };

private rel[Symbol,Symbol] getDependencies(s1: \opt(Symbol s2)) = <s1, removeOuterLabel(s2)> + getDependencies(s2);
private rel[Symbol,Symbol] getDependencies(s1: \iter(Symbol s2)) = <s1, removeOuterLabel(s2)> + getDependencies(s2);
private rel[Symbol,Symbol] getDependencies(s1: \iter-star(Symbol s2)) = <s1, removeOuterLabel(s2)> + getDependencies(s2);
private rel[Symbol,Symbol] getDependencies(s1: \iter-star-seps(Symbol s2, list[Symbol] separators)) = 
    {<s1, removeOuterLabel(sym)> | Symbol sym <- separators} + { *getDependencies(sym) | Symbol sym <- separators };

private rel[Symbol,Symbol] getDependencies(s1: \alt(set[Symbol] alternatives)) = 
    {<s1, removeOuterLabel(sym)> | Symbol sym <- alternatives} + { *getDependencies(sym) | Symbol sym <- alternatives };

private rel[Symbol,Symbol] getDependencies(s1: \seq(list[Symbol] symbols)) = 
    {<s1, removeOuterLabel(sym)> | Symbol sym <- symbols} + { *getDependencies(sym) | Symbol sym <- symbols };

// TODO Condition and other possible cases are missing here

private rel[Symbol,Symbol] getDependencies(s1: \conditional(Symbol symbol, set[Condition] conditions)) = 
    {<s1, removeOuterLabel(sym)> | Symbol sym <- getSymbols(conditions)} + { *getDependencies(sym) | Symbol sym <- getSymbols(conditions) };

private set[Symbol] getSymbols(set[Condition] conditions) =
    { c.symbol | c <- conditions, c has symbol };

// Remove outermost label

private Symbol removeOuterLabel(Symbol s1) = Symbol::\label(_,Symbol s2) := s1 ? s2 : s1;

// TODO: alias, func, var-func, reified

private default rel[Symbol,Symbol] getDependencies(Symbol s) = {};
...
private bool isParameterFree(list[Symbol] symbols) = !(/\parameter(name, formals) := symbols);

// Get all instantiated parameters in the complete grammar

private set[Symbol] getInstantiatedParameters(){
	instantiated_params = {};
	visit(cachedGrammar){
...
// 
private map[Symbol, Production] instantiateAllParameterizedProductions(){
	instantiated_params = getInstantiatedParameters();
	instantiated_productions = ();
	for(Symbol lhs <- cachedGrammar){
...
private  tuple[set[Symbol], set[Production]] getReachableAbstractTypes(Symbol subjectType, set[str] consNames, set[Symbol] patternTypes){
	...
	//	descend_into = {\value()};
	//}
	tuples = { Symbol::\tuple(symbols) | sym <- descend_into, \rel(symbols) := sym || \lrel(symbols) := sym };
	descend_into += tuples;
...
private type[value] symbolToValue1(Symbol symbol) {
