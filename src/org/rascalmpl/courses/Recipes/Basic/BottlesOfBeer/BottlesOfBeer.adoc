...
<2> _Pattern-directed invocation_ (see link:{RascalLang}#Declarations-Function[function declaration]) will determine at the call site which function will be called. The
...
<3> The main function is `sing` that iterates over the numbers 99 downto 1 (as described by the range `[99 .. 1]`)
