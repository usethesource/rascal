...
We exploit the subtype relation (see link::{Rascal}#Concepts-StaticTyping[]) between link::{Rascal}#AlgebraicDataType[]s and the type link::{Rascal}#Values-Node[] to achieve this.
...
<4> Defines the case that we encounter a node and we update its frequency count.
  First the name of the constructor is retrieved (using link::{Rascal}#getName[]) and then the
  frequency is updated. The link::{Rascal}#Statements-Assignment-IsDefined[] operator is used to provide a default value of 0 when
...
names that is used to filter the map that is returned by `count` using link::{Rascal}#Map-domainR[].
