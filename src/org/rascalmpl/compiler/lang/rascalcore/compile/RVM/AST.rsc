...
public data RVMProgram =
            rvmProgram(
                ...
                map[str,AType] imported_types,
                list[RVMDeclaration] imported_declarations,
                 map[str,int] imported_overloading_resolvers,
                lrel[str name, AType funType, str scope, list[str] ofunctions, list[str] oconstructors] imported_overloaded_functions
                )
         ; 
...
public data Instruction =
        ...
        | LOADINT(int nval)  						// Push a (Java) integer
	   	...
	   	| LOADTYPE(AType \type)					// Load a type constant in accu
	   	| PUSHTYPE(AType \type)                    // Push a type constant
		...
		| OCALLDYN(AType types, int arity, loc src)// Call a *Rascal* function on stack
...
//		| CALLMUPRIM(str name, int arity)			// Call a muRascal primitive (see Compiler.RVM.Interpreter.MuPrimitive) 
		...
		| CHECKARGTYPEANDCOPY(
			int pos1, AType \type, int pos2)		// Check the type of argument at pos1 and assign to pos2
		...
		| VALUESUBTYPE(AType \type)                // Check that type of top element is subtype of given type
;
