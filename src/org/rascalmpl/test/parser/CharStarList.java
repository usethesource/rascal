...
public class CharStarList extends SGLL implements IParserTest{
	...
	public IValue executeParser(){
		...
		return csl.parse(NONTERMINAL_START_S, "abc".toCharArray());
	}
	
	public IValue getExpectedResult() throws IOException{
		String expectedInput = "parsetree(appl(prod([\\iter-star(\\char-class([range(97,122)]))],sort(\"S\"),\\no-attrs()),[appl(regular(\\iter-star(\\char-class([range(97,122)]))),[char(97),appl(regular(\\iter-star(\\char-class([range(97,122)]))),[char(98),appl(regular(\\iter-star(\\char-class([range(97,122)]))),[char(99)])])])]),-1)";
