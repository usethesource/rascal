...
void compileMuLibrary(list[loc] srcPath, list[loc] libPath, loc bootLoc, loc binDir, bool verbose = false, bool jvm=true) {
  compileMuLibrary(pathConfig(srcPath=srcPath, libPath=libPath, bootLoc=bootLoc, binDir=binDir), verbose=verbose, jvm=jvm);
}