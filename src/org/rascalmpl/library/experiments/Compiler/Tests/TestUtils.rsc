...
PathConfig pcfg = pathConfig(srcPath=[|test-modules:///|, |std:///|]);

value run(str exp, bool debug=false, bool debugRVM=false, bool recompile=true, bool profile=false) {
    TMP = makeTMP();
    msrc = "module TMP data D = d1(int n, str s) | d2(str s, bool b) | d3(list[int] l, list[int] r); value main() = <exp>;";
	writeFile(TMP, msrc);
	return execute(TMP, pcfg, debug=debug, debugRVM=debugRVM, recompile=recompile, profile=profile);
}	
value run(str before, str exp, bool debug=false, bool debugRVM=false,  bool recompile=true, bool profile=false) {
   TMP = makeTMP();
...
value run(str exp, list[str] imports, bool debug=false, bool debugRVM=false, bool recompile=true, bool profile=false) {
   TMP = makeTMP();
