...
public class CodeBlock {
	 ...
	 * 
	 * argument2 argument1 op |-----------| |-----------| |---------| sizeArg2 bits sizeArg1 bits sizeOp bits
	 */
	...
	public CodeBlock CALLJAVA(String methodName, String className, Type parameterTypes, Type keywordTypes, int reflect) {
		return add(new CallJava(this, getConstantIndex(vf.string(methodName)), getConstantIndex(vf.string(className)), getTypeConstantIndex(parameterTypes),
				getTypeConstantIndex(keywordTypes), reflect));
	}
	...
	public CodeBlock done(BytecodeGenerator codeEmittor, String fname, Map<String, Integer> codeMap, Map<String, Integer> constructorMap, Map<String, Integer> resolver,
			boolean listing, boolean debug) {
		this.functionMap = codeMap;
		this.constructorMap = constructorMap;
		this.resolver = resolver;
		int codeSize = pc;
...
class RascalValuesValueFactory extends AbstractValueFactoryAdapter {
	...
	private IConstructor specializeType(String name, IValue... children) {
		if ("type".equals(name) && children.length == 2 && children[0].getType().isSubtypeOf(Factory.Type_Reified.getFieldType(0))
