...
public class RVMV2 implements IRVM {
	...
	public RVMV2(RascalExecutionContext rex) {
		super();
	...
	Configuration getConfiguration() {
		return rex.getConfiguration();
	}

	List<ClassLoader> getClassLoaders() {
		return rex.getClassLoaders();
	}
...
//					}
				...
				case Opcode.OP_LOADCON:
					insnLOADCON(cf,stack,sp++,CodeBlock.fetchArg1(instruction));
					continue NEXT_INSTRUCTION;
				...
				case Opcode.OP_CHECKARGTYPEANDCOPY:
					sp = insnCHECKARGTYPEANDCOPY(cf, stack, sp, CodeBlock.fetchArg1(instruction), CodeBlock.fetchArg2(instruction), instructions[pc++]);
//					pos = CodeBlock.fetchArg1(instruction);
...
//						precondition = (Boolean) rval;
	...
	HashSet<String> converted = new HashSet<String>(Arrays.asList("org.rascalmpl.library.experiments.Compiler.RVM.Interpreter.ParsingTools.parseFragment",
			...
			"org.rascalmpl.library.PreludeCompiled.implode", "org.rascalmpl.library.PreludeCompiled.parse", "org.rascalmpl.library.PreludeCompiled.print",
			"org.rascalmpl.library.PreludeCompiled.println", "org.rascalmpl.library.PreludeCompiled.iprint", "org.rascalmpl.library.PreludeCompiled.iprintln",
			"org.rascalmpl.library.PreludeCompiled.rprint", "org.rascalmpl.library.PreludeCompiled.rprintln", "org.rascalmpl.library.util.MonitorCompiled.startJob",
			"org.rascalmpl.library.util.MonitorCompiled.event", "org.rascalmpl.library.util.MonitorCompiled.endJob", "org.rascalmpl.library.util.MonitorCompiled.todo",
			"org.rascalmpl.library.util.ReflectiveCompiled.getModuleLocation", "org.rascalmpl.library.util.ReflectiveCompiled.getSearchPathLocation",
			"org.rascalmpl.library.util.ReflectiveCompiled.inCompiledMode", "org.rascalmpl.library.util.ReflectiveCompiled.diff", "org.rascalmpl.library.util.ReflectiveCompiled.watch"
	...
	Frame root;
	int[] instructions;
	OverloadedFunctionInstanceCall c_ofun_call;
	Stack<OverloadedFunctionInstanceCall> ocalls = new Stack<OverloadedFunctionInstanceCall>();
	int postOp;
	Thrown thrown;
	ArrayList<Frame> stacktrace = new ArrayList<Frame>();
	...
	public void insnLOADLOC() {
		int pos = CodeBlock.fetchArg1(instruction);
		assert stack[pos] != null : "Local variable " + pos + " is null";
		stack[sp++] = stack[pos];
		return;
	...
	public void insnLOADBOOL(Object[] stack, int sp, int bVal) {
	...
	public void insnLOADCON(Frame cf, Object[] stack, int sp, int con) {
		stack[sp] = cf.function.constantStore[con];
	}
	...
	public void insnLOADTYPE() {
		stack[sp++] = cf.function.typeConstantStore[CodeBlock.fetchArg1(instruction)];
		return;

	}

	public void insnLOADLOCDEREF() {
		Reference ref = (Reference) stack[CodeBlock.fetchArg1(instruction)];
		stack[sp++] = ref.stack[ref.pos];
		return;
	}

	public void insnSTORELOC() {

	}

	public void insnUNWRAPTHROWNLOC() {
		stack[CodeBlock.fetchArg1(instruction)] = (op == Opcode.OP_STORELOC) ? stack[sp - 1] : ((Thrown) stack[--sp]).value;
		return;
	}

	public void insnSTORELOCDEREF() {
		Reference ref = (Reference) stack[CodeBlock.fetchArg1(instruction)];
		ref.stack[ref.pos] = stack[sp - 1]; // TODO: We need to re-consider how to guarantee safe use of both Java objects and IValues
		return;
	...
	public void insnLOAD_NESTED_FUN() {
		// Loads nested functions and closures (anonymous nested functions)
		stack[sp++] = FunctionInstance.computeFunctionInstance(functionStore.get(CodeBlock.fetchArg1(instruction)), cf, CodeBlock.fetchArg2(instruction), this);
		return;
	}

	public void insnLOADOFUN() {
		OverloadedFunction of = overloadedStore.get(CodeBlock.fetchArg1(instruction));
	...
	public void insnLOADVAR() {

	}

	public void insnLOADVARREF() {
		int varScope = CodeBlock.fetchArg1(instruction);
		int pos = CodeBlock.fetchArg2(instruction);
	...
	public void insnSTOREVAR() {

	}

	public void insnUNWRAPTHROWNVAR() {
		int s = CodeBlock.fetchArg1(instruction);
		int pos = CodeBlock.fetchArg2(instruction);
	...
	public void insnCALLCONSTR() {
		Type constructor = constructorStore.get(CodeBlock.fetchArg1(instruction));
		int arity = CodeBlock.fetchArg2(instruction);
		// cf.src = (ISourceLocation) cf.function.constantStore[instructions[pc++]];
	...
	public void insnCALLDYN() {

	}

	public void insnCALL() {
	...
	public void insnOCALLDYN() {

	}

	public void insnOCALL() {
		Object funcObject = (op == Opcode.OP_OCALLDYN) ? stack[--sp] : null;
		// Get function arguments from the stack
		int arity = CodeBlock.fetchArg2(instruction);
		...
		if (frame != null) {
			...
			Type constructor = c_ofun_call_next.nextConstructor(constructorStore);
			sp = sp - arity;
			stack[sp++] = vf.constructor(constructor, c_ofun_call_next.getConstructorArguments(constructor.getArity()));
		}
		return;
	...
	public int insnCHECKARGTYPEANDCOPY(Frame cf, Object[] stack, int sp, int pos, int ptype, int pos2) {
		Type argType = ((IValue) stack[pos]).getType();
		Type paramType = cf.function.typeConstantStore[ptype];
	...
	public void insnCREATE() {

	}

	public void insnCREATEDYN() {
		if (op == Opcode.OP_CREATE) {
			cccf = cf.getCoroutineFrame(functionStore.get(CodeBlock.fetchArg1(instruction)), root, CodeBlock.fetchArg2(instruction), sp);
		} else {
			int arity = CodeBlock.fetchArg1(instruction);
	...
	public void insnAPPLY() {

	}

	public void insnAPPLYDYN() {
		FunctionInstance fun_instance;
		int arity;
		if (op == Opcode.OP_APPLY) {
			Function fun = functionStore.get(CodeBlock.fetchArg1(instruction));
	...
	public void insnRESET() {
		FunctionInstance fun_instance = (FunctionInstance) stack[--sp]; // A function of zero arguments
		cf.pc = pc;
		cf = cf.getCoroutineFrame(fun_instance, 0, sp);
		activeCoroutines.push(new Coroutine(cf));
		ccf = cf;
	...
	public void insnSHIFT() {
		FunctionInstance fun_instance = (FunctionInstance) stack[--sp]; // A function of one argument (continuation)
		Coroutine coroutine = activeCoroutines.pop();
		ccf = activeCoroutines.isEmpty() ? null : activeCoroutines.peek().start;
		cf.pc = pc;
		cf.sp = sp;
