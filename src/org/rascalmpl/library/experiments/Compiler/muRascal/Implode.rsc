        ...
        try {
          append
            top-down visit(exp){
               ...
               case muCall(preVar("starts_with"), [exp1, exp2, exp3])		
               														=> muCallMuPrim("starts_with", [exp1, exp2, exp3])
               ...
               case muCall(preVar("mset_subtract_mset"), list[MuExp] exps)		
               														=> muCallMuPrim("mset_subtract_mset", exps)
               	case muCall(preVar("mset_subtract_elm"), list[MuExp] exps)		
               														=> muCallMuPrim("mset_subtract_elm", exps)
               case muCall(preVar("mset_subtract_set"), list[MuExp] exps)		
               														=> muCallMuPrim("mset_subtract_set", exps)
               case muCall(preVar("set_subtract_elm"), list[MuExp] exps)		
               														=> muCallPrim("set_subtract_elm", exps)
               ...
               case muCall(preVar("mset_add_elm"), list[MuExp] exps)		=> muCallMuPrim("mset_add_elm", exps)
