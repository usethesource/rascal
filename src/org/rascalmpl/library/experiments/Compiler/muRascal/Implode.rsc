        ...
        try {
          append
            top-down visit(exp){
               ...
               case muCall(preVar("get_name_and_children"), [exp1])
                											=> muCallMuPrim("get_name_and_children", [exp1])
               ...
               case muCall(preVar("sublist"), list[MuExp] exps)	
               												=> muCallMuPrim("sublist_list_mint_mint", exps)
               case muCall(preVar("get_tuple_elements"), [exp1])
               												=> muCallMuPrim("get_tuple_elements", [exp1])
               case muCall(preVar("println"), list[MuExp] exps)		
               												=> muCallMuPrim("println", exps)
