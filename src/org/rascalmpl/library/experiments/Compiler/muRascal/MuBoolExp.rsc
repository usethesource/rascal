...
data MuExp 
    = muBoolSingle(str operator, list[MuExp] exps, loc src)
    | muBoolMulti(str operator, list[MuExp] exps, loc src)
    ;

tuple[MuExp, list[MuFunction]] makeBoolExp(str operator, str scope, list[MuExp] exps, loc src) = <muBoolMulti(operator, exps, src), []>;

tuple[MuExp,list[MuFunction]] makeMultiValuedBoolExp(str operator, str fuid, list[MuExp] exps, loc src)
    = <muBoolMulti(operator, exps, src), []>;
    
tuple[MuExp,list[MuFunction]] makeSingleValuedBoolExp(str operator, str fuid, list[MuExp] exps, loc src)
    = <muBoolSingle(operator, exps, src), []>;
...
//
//default MuExp makeBoolExp(str operator, str scope, list[MuExp] exps, loc src) = muBool(operator, exps, src);
//
//default tuple[MuExp,list[MuFunction]] makeBoolExp(str operator, str fuid, list[MuExp] exps, loc src) {
...
//
//    functions += muFunction(all_uid, "RASCAL_ALL", Symbol::func(\int(), [], []), [], Symbol::\tuple([]), fuid, 0, size(localvars), false, false, true, src, [], (), false, 0, 0, muBlock(body));
