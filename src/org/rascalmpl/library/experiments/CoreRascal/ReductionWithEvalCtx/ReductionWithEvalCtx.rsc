...
public Exp step( C(Exp::apply(Exp::lambda(str id, Exp exp1), Exp exp2), Ctx::config(ctx, Store store)) ) = { store[id] = exp2; C(exp2, Ctx::config(ctx, store_)); } when isValue(exp2);
...
public Exp step( C(Exp::labeled(str l, Exp exp), Ctx::config(ctx1, Store store)) ) = { store[l] = Exp::lambda("xvar", plug(Exp::id("xvar"), ctx2)); C(e, Ctx::config(ctx1,store)); } when !isValue(exp) && C(Exp::yield(Exp e), Ctx ctx2) := exp && !Ctx::config(_,_) := ctx2 && isValue(e);
...
public Exp step( C(Exp::abort(Exp exp), Ctx ctx) ) = exp;
public Exp step( C(Exp::callcc(Exp exp), Ctx ctx) ) = C(Exp::apply(exp, Exp::lambda("xvar", Exp::abort(C(Exp::id("xvar"), ctx)))), ctx) when isValue(exp);
