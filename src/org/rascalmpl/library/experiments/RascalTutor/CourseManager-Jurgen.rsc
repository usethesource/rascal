...
public str validateAnswer1(map[str,str] params) {
  if (params["cheat"] == "yes") return showCheat(cpid, qid, q, params);
  if (params["another"] == "yes") showAnother(cpid, qid, q);
  
  qid = params["exercise"];
  cpid = params["concept"];
  lastQuestion = qid;
  
  return validateAnswer(getQuestion(cpid,qid),trim(params["answer"]) ? "",params["exp"] ? "");
}

public str validateAnswer1(choiceQuestion(cid,qid,descr,choices), str answer, str expr) {
  try {
    int c = toInt(answer);
...
public str validateAnswer1(textQuestion(cid,qid,descr,replies), str answer, str expr) 
  = (toLowerCase(answer) in replies) ? correctAnswer(cpid, qid) : wrongAnswer(cpid, qid, "");
 
public str validateAnswer1(tvQuestion(cid, qid, qkind, qdetails), str answer, str expr) {
...
public str validateAnswer1(typeOfExpr(), str answer, str expr) {
      try {
        if(lstBefore == ""){ // Type question without listing
           ...
           if (answerType == expectedType) {
             ...
             if(!holeInCnd){
               try 
                 ...
                 errorMsg = "I expected the answer <expectedType>; \"<answer>\" is not a legal Rascal type.";
             }
          ...
          //println("Evaluating validate: <validate>");
          try {
             output = eval(setup + validate);
             expectedType = toString(generateType(rtype, env));
             
             if (((holeInLst || holeInCnd) ? answerType : answer) == expectedType) {
               return correctAnswer(cpid, qid);
             }
             else {
               errorMsg = "I expected the answer <expectedType> instead of <answerType>.";
               ...
               errorMessage = wrongAnswer(cpid, qid, errorMsg);
               errorMsg += (holeInLst) ? "I expected a value of type <expectedType>. "
                                       : "I expected the answer <expectedType>. ";
             }
          }
          catch ParseError(loc l): {
            errorMsg = "There is a syntax error in your answer on line <l.begin.line>, column <l.begin.column> ";
          }
          catch StaticError(str msg, loc l): {
             errorMsg = "There is a static error in your answer (<msg>) on line <l.begin.line>, column <l.begin.column> ";
          }
          catch value x : {
              errorMsg = "There is was an error (<x>)";
          }
...
public str validateAnswer1(Question _, str answer, str expr) { 
