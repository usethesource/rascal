...
public class NewTree extends Compose {
	...
	public NewTree(Dimension major,Figure[] figures, PropertyManager properties) {
	...
	Coordinate getChildCenter(int i){
		if(children[i] instanceof NewTree){
			NewTree subTree = (NewTree)children[i];
	...
	Outline getLeftOutline(Figure fig){
		if(fig instanceof NewTree){
			NewTree subTree = (NewTree)fig;
