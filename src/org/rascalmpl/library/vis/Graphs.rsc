...
@description{
This module provides a simple API to create graph visuals for Rascal
...
data CytoLayout(CytoLayoutName name = dagre(), bool animate=false)
    ...
    /* dotLayout is special: instead of using Cytoscape, the Cytoscape graph is converted into a dot graph and the wasm version of graphviz is used.
       Although the Cytosscape "dagre" layout uses the graphviz algorithm, it does not expose the edge coordinates generated by graphviz.
       The edge routing done by graphviz is superior to the Cytoscape algorithm, especially for hierarchical graphs as used by vis::ValueGraph to visualize data structures.
       Hence the introduction of this special layout.
    */
    | dotLayout(
        CytoLayoutName name = dot(),
        num rankSep = 0.3,
        str rankDir = "TD",
        str bgColor = "white"
    )
...
CytoLayout defaultDotLayout(num rankSep=0.3, str rankDir="TD", str bgColor="white")
    = dotLayout(
        name=CytoLayoutName::dot(),
        rankSep = rankSep,
        rankDir = rankDir,
        bgColor = bgColor
    );
...
Response (Request) graphServer(Cytoscape ch, str pageTitle="Graph") {
    ...
    default Response reply(get(_)) {
        HTMLElement html = dotLayout() := ch.\layout ? plotDotHTML(pageTitle) : plotCytoHTML(pageTitle);
        return response(writeHTMLString(html));
    }

    // Work around a bug in the typechecker for now
    //return reply;
    Response (Request) returnReply = reply;
    return returnReply;
}
...
private HTMLElement plotCytoHTML(str pageTitle)
    = html([
        head([ 
