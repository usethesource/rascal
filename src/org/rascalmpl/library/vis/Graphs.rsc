...
Response (Request) graphServer(Cytoscape ch, bool useDotRenderer=false) {
    ...
    default Response reply(get(_)) {
        return response(writeHTMLString(useDotRenderer ? plotDotHTML() : plotCytoscapeHTML()));
    }
...
private HTMLElement plotCytoscapeHTML()
    = html([
        head([ 
...
private HTMLElement plotDotHTML()
    = html([
        head([ 
