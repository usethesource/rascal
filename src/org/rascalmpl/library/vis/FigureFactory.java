...
public class FigureFactory {
	final static IValueFactory vf = ValueFactoryFactory.getValueFactory();
	final static IList emptyList = vf.list();

	enum Primitives {
		BOX, BUTTON, CHECKBOX, CHOICE, COMBO, COMPUTEFIGURE, CONTROLON, CONTROLOFF, EDGE, ELLIPSE, GRAPH, GRID, HAXIS, HCAT, HSCREEN, HVCAT, OUTLINE, OVERLAY, PACK, PLACE, PROJECTION, ROTATE, SCALE, SHAPE, SPACE, TEXT, TEXTFIELD, TREE, TREEMAP, USE, VAXIS, VCAT, VERTEX, VSCREEN, WEDGE, XAXIS
	}
	...
	public static Figure[] makeList(IFigureApplet fpa, IValue list,
	...
	public static Figure makeChild(IFigureApplet fpa, IConstructor c,
			PropertyManager properties, IList childProps, IEvaluatorContext ctx) {
		if (c.arity() == 2) {
	...
	public static Figure makeChild(int index, IFigureApplet fpa,
			IConstructor c, PropertyManager properties, IList childProps,
			IEvaluatorContext ctx) {
		return FigureFactory.make(fpa, (IConstructor) c.get(0), properties,
				childProps, ctx);
	}
	...
	public static Figure make(IFigureApplet fpa, IConstructor c,
			PropertyManager properties, IList childProps, IEvaluatorContext ctx) {
		String ename = c.getName();
		properties = PropertyManager.extendProperties(fpa, c, properties,
				childProps, ctx);
		IList childPropsNext = PropertyManager.getChildProperties((IList) c
				.get(c.arity() - 1));
		if (childProps != null) {
			IList childchildProps = PropertyManager
					.getChildProperties(childProps);
		...
		if (fpa.getComp() == null) {
			// In case of printing environment
			switch (pmap.get(ename)) {
			case BUTTON:
			case CHECKBOX:
			case COMBO:
				return new Text(fpa, properties,
						((IString) c.get(0)).getValue(), true);
			case CHOICE:
				return new Text(fpa, properties, "choice", true);
			}
		}
		Figure[] children;
		IValue validate;
		...
		case BOX:
			return new Box(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);

		case BUTTON:
			return new Button(fpa, ((IString) c.get(0)).getValue(), c.get(1),
					ctx, properties);

		case CHECKBOX:
			return new Checkbox(fpa, ((IString) c.get(0)).getValue(), c.get(1),
					ctx, properties);

		case CHOICE:
			return new Choice(fpa, makeStringList((IList) c.get(0)), c.get(1),
					ctx, properties);
		...
		case COMBO:
			...
			return new Combo(fpa, ((IString) c.get(0)).getValue(),
					makeStringList((IList) c.get(1)), c.get(2), validate, ctx,
					properties);
		...
		case ELLIPSE:
			return new Ellipse(fpa, makeChild(fpa, c, properties,
					childPropsNext, ctx), properties);

		case GRAPH:
			if (properties.getStringProperty(StrProp.HINT).contains("lattice"))
				return new LatticeGraph(fpa, properties, (IList) c.get(0),
						(IList) c.get(1), ctx);
			if (properties.getStringProperty(StrProp.HINT).contains("layered"))
				return new LayeredGraph(fpa, properties, (IList) c.get(0),
						(IList) c.get(1), ctx);
			return new SpringGraph(fpa, properties, (IList) c.get(0),
					(IList) c.get(1), ctx);
		...
		case HAXIS:
			return new HAxis(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);
		...
		case HSCREEN:
			return new HScreen(fpa, makeChild(fpa, c, properties,
					childPropsNext, ctx), properties);
		...
		case OUTLINE:
			return new Outline(fpa, properties, (IList) c.get(0),
					(IInteger) c.get(1));
		...
		case PLACE:
			return new Place(fpa, properties, (IConstructor) c.get(0),
					(IString) c.get(1), (IConstructor) c.get(2), ctx);
		...
		case PROJECTION:
			...
			Figure projecton = makeChild(projectionIndex, fpa, c, properties,
					childPropsNext, ctx);
			Figure projection = makeChild(0, fpa, c, properties,
					childPropsNext, ctx);
			return new Projection(fpa, name, projecton, projection, properties);
		case ROTATE:
			// TODO
			return new Rotate(fpa, properties, c.get(0),
					(IConstructor) c.get(1), ctx);

		case SCALE:
			// TODO
			if (c.arity() == 3)
				return new Scale(fpa, properties, c.get(0), c.get(0),
						(IConstructor) c.get(1), ctx);

			return new Scale(fpa, properties, c.get(0), c.get(1),
					(IConstructor) c.get(2), ctx);
		...
		case SPACE:
			return new Space(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);

		case TEXT:
			IPropertyValue<String> txt = new PropertyParsers.StringArgParser()
					.parseProperty(StrProp.TEXT, c, null, 0, fpa, ctx);
		...
		case TEXTFIELD:
			...
			if (c.arity() > 3)
				validate = c.get(2);
			return new TextField(fpa, ((IString) c.get(0)).getValue(),
					c.get(1), null, ctx, properties);

		case TREE:
			return new Tree(fpa, properties, (IList) c.get(0),
					(IList) c.get(1), ctx);

		case TREEMAP:
			return new TreeMap(fpa, properties, (IList) c.get(0),
					(IList) c.get(1), ctx);

		case USE:
			return new Use(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);

		case VAXIS:
			return new VAxis(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);
		...
		case VSCREEN:
			return new VScreen(fpa, makeChild(fpa, c, properties,
					childPropsNext, ctx), properties);

		case VERTEX:
			return new Vertex(fpa, properties, c.get(0), c.get(1),
					c.arity() == 4 ? (IConstructor) c.get(2) : null, ctx);

		case WEDGE:
			return new Wedge(fpa, makeChild(fpa, c, properties, childPropsNext,
					ctx), properties);
		}
	...
	public static SpringGraphEdge makeSpringGraphEdge(SpringGraph G,
			IFigureApplet fpa, IConstructor c, PropertyManager properties,
			IEvaluatorContext ctx) {
		...
		return new SpringGraphEdge(G, fpa, properties, from, to, toArrow,
				fromArrow, ctx);
	}

	public static LayeredGraphEdge makeLayeredGraphEdge(LayeredGraph G,
			IFigureApplet fpa, IConstructor c, PropertyManager properties,
			IEvaluatorContext ctx) {
		IString from = (IString) c.get(0);
		IString to = (IString) c.get(1);
		// IConstructor toArrow = c.arity() > 3 ? (IConstructor) c.get(2) :
		// null;
		// IConstructor fromArrow = c.arity() > 4 ? (IConstructor) c.get(3) :
		// null;
		return new LayeredGraphEdge(G, fpa, properties, from, to, ctx);
	}

	public static LatticeGraphEdge makeLatticeGraphEdge(LatticeGraph G,
			IFigureApplet fpa, IConstructor c, PropertyManager properties,
			IEvaluatorContext ctx) {
		IString from = (IString) c.get(0);
		IString to = (IString) c.get(1);
