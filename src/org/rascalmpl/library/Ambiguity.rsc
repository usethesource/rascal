...
public list[Message] findCauses(Tree a) {
  return [info("Ambiguity cluster with <size(a.alternatives)> alternatives", a.origin)]
...
public list[Message] findCauses(Tree x, Tree y) {
  ...
  if (pX == pY) {
    result += [info("The alternatives use the same productions", x.origin)];
  }
  else {
      result += [info("Production unique to the one alternative: <alt2rascal(p)>;", x.origin) | p <- pX - pY];
      result += [info("Production unique to the other alternative: <alt2rascal(p)>;", x.origin) | p <- pY - pX];
  }  
  
  if (appl(prodX,_) := x, appl(prodY,_) := y) {
    if (prodX == prodY) {
    result += [info("The alternatives have the same production at the top: <alt2rascal(prodX)>", x.origin)];
    } 
    else {
      result += [info("The alternatives have different productions at the top, one has 
                      '  <alt2rascal(prodX)>
                      'while the other has
                      '  <alt2rascal(prodY)>", x.origin)];
    }
  }
  ...
  if (appl(p, argsX) := x, appl(q, argsY) := y) {
    if (i <- index(argsX), appl(apX,_) := argsX[i], apX notin pY) {
      ...
      result += [error("To fix this issue, you could restrict the nesting of
                       ...
                       'However, you should realize that you are introducing a restriction that makes the language smaller",x.origin)];
    }
...
public list[Message] deeperCauses(Tree x, Tree y, set[Production] pX, set[Production] pY) {
  ...
  if (rX<0> != rY<0> || lX<1> != lY<1>) {
    result += [info("The alternatives have different lexicals/literals/layout", x.origin )];
    result += [info("Unique lexical to the one: <alt2rascal(p)>;", t[0].origin) | t <- (rX - rY), p := t[0].prod];
    result += [info("Unique lexical to the other: <alt2rascal(p)>;", t[0].origin) | t <- (rY - rX), p := t[0].prod];
    result += [info("Unique literal to the one: <symbol2rascal(t[1].prod.def)>", x.origin) | t <- lX - lY];
    result += [info("Unique literal to the other: <symbol2rascal(t[1].prod.def)>", x.origin) | t <- lY - lX];
    result += [info("Unique layout to the one: <symbol2rascal(t[0].prod.def)>", x.origin) | t <- laX - laY];
    result += [info("Unique layout to the other: <symbol2rascal(t[0].prod.def)>", x.origin) | t <- laY - laX];
    
    // literals that became lexicals and vice versa
    result += [error("You might reserve <l> from <symbol2rascal(r.prod.def)>, i.e. using a reject (reserved keyword).", r.origin) | <r,l> <- rX o lY];
    result += [error("You might reserve <l> from <symbol2rascal(r.prod.def)>, i.e. using a reject (reserved keyword).", r.origin) | <r,l> <- rY o lX];
  ...
  if (overloadedLits != []) {
    result += info("Re-use of these literals is causing different interpretations of the same source.", x.origin);
    ...
    for (<p,i,q> <- (fatherChildX - fatherChildY) + (fatherChildY - fatherChildX)) {
      ...
      if (prod(label(l,_),_,_) := q) {
        ...
        result += [warning("You should give this production a good label [<alt2rascal(q)>]",x.origin)];
      }
      ...
      result += [error("You could safely restrict the nesting of
                       ...
                       'under
                       '  <alt2rascal(p)>
                       'using the ! operator on argument <i/2>: !<labelApX>",x.origin)];
    } 
  }
...
list[Message] priorityCauses(Tree x, Tree y) {
  if (/appl(p,[appl(q,_),_*]) := x, /Tree t:appl(q,[_*,appl(p,_)]) := y, p != q) {
      return [error("You might add this priority rule (or vice versa):
                    '  <alt2rascal(priority(p.def,[p,q]))>", t.origin)
             ,error("You might add this associativity rule (or right/assoc/non-assoc):
                    '  <alt2rascal(associativity(p.def, \left(), {p,q}))>", t.origin)];
  }
...
list[Message] danglingOffsideSolutions(Tree x, Tree y) {
  if (appl(p,/Tree u:appl(q,_)) := x, appl(q,/appl(p,_)) := y
     , (u.origin).begin.column >= (x.origin).begin.column
     , (u.origin).begin.line < (x.origin).end.line) {
    return [error("You might declare nested <prod2rascal(q)> offside (to the left) of some child of <prod2rascal(p)> using a failing syntax action that compares annotations @loc.start.column", u.origin)];
  }
  
  if (appl(p,/Tree u:appl(q,_)) := y, appl(q,/appl(p,_)) := x
     , (u.origin).begin.column >= (y.origin).begin.column
     , (u.origin).begin.line < (y.origin).end.line) {
    return [error("You might declare nested <prod2rascal(q)> offside (to the left) of some child of <prod2rascal(p)> using a failing syntax action that compares annotations @loc.start.column", u.origin)];
  }
...
list[Message] associativityCauses(Tree x, Tree y) {
  if (/appl(p,[appl(p,_),_*]) := x, /Tree t:appl(p,[_*,appl(p,_)]) := y) {
    return [error("This rule [<alt2rascal(p)>] may be missing an associativity declaration  (left, right, non-assoc)", t.origin)];
  }
