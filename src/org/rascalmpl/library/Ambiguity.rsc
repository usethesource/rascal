...
public list[Message] deeperCauses(Tree x, Tree y) {
  ...
  if (rX<0> != rY<0> || lX<1> != lY<1>) {
    ...
    result += [info("Unique literal to the one: <symbol2rascal(t[1].prod.rhs)>", x@\loc) | t <- lX - lY];
    result += [info("Unique literal to the other: <symbol2rascal(t[1].prod.rhs)>", x@\loc) | t <- lY - lX];
    result += [info("Unique layout to the one: <symbol2rascal(t[0].prod.rhs)>", x@\loc) | t <- laX - laY];
    result += [info("Unique layout to the other: <symbol2rascal(t[0].prod.rhs)>", x@\loc) | t <- laY - laX];
    
    // literals that became lexicals and vice versa
    result += [error("You might reserve <l> from <symbol2rascal(r.prod.rhs)>, i.e. using a reject (reserved keyword).", r@\loc) | <r,l> <- rX o lY];
    result += [error("You might reserve <l> from <symbol2rascal(r.prod.rhs)>, i.e. using a reject (reserved keyword).", r@\loc) | <r,l> <- rY o lX];
  ...
  polX = {<p,l[@\loc=t@\loc]> | /t:appl(p,[_*,b,_,l:appl(prod(_,lit(_),_),_),_*]) := x, true}; 
  polY = {<l[@\loc=t@\loc],p> | /t:appl(p,[_*,b,_,l:appl(prod(_,lit(_),_),_),_*]) := y, true};
