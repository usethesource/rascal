...
public value eval0(str main, list[int] args, Prog prog) { /*2*/
...
public int eval0(nat(int nat), PEnv penv)  = nat; /*4*/
...
public int eval0(cond(Exp cond, Exp then, Exp otherwise), PEnv penv) =  /*5*/
           (eval0(cond, penv) != 0) ? eval0(then, penv) : eval0(otherwise, penv);
          
       
public int eval0(call(str name, list[Exp] args), PEnv penv) =  /*6*/
