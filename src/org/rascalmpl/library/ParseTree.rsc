...
The full definition of `Tree`, `Production` and `Symbol` is given in [Tree]. A parse tree is a nested tree structure of type `Tree`. 
...
// Provides access to the source location of a parse tree node.
anno loc Tree@\loc;
