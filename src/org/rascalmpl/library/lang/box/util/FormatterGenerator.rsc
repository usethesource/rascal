...
str(type[Tree], str) subStringFormatter(type[&G <: Tree] grammar, Style style) {
    list[TextEdit](type[Tree], str) toEdits = subStringEdits(grammar, style);
...
list[TextEdit](&G <: Tree) treeEdits(type[&G <: Tree] grammar, Style style) {
    &G(value, loc) p = parser(grammar);

    return list[TextEdit] (&G <: Tree tree) {
        try {
            return layoutDiff(tree, p(format(style(tree)), tree@\loc.top));
        }
        catch ParseError(loc place): { 
...
list[TextEdit](type[Tree], str) subStringEdits(type[&G <: Tree] grammar, Style style) {
...
void debugFileFormat(type[&G <: Tree] grammar, Style style, loc input, bool HTML=true, bool console=!HTML) {
...
void debugFilesFormat(type[&G <: Tree] grammar, Style style, loc root, str extension, bool shadowFiles = true, bool appendFile=!shadowFiles, bool console=!shadowFiles && !appendFile, bool ansi=console || appendFile) {
...
void debugStringFormat(type[&G <: Tree] grammar, Style style, str input, bool HTML = true, bool console = !HTML) {
    str(str) formatter = stringFormatter(grammar, style);
