...
data PatternTree(
  ...
  Symbol typeHint = Symbol::\value()) 
    ...
    | spliceNodePlus(RName name, loc eAt, Symbol etype, int nameId)
    | spliceNodeStar(RName name, int nameId)
    | spliceNodeStar(RName name, loc eAt, Symbol etype, int nameId)
    | negativeNode(PatternTree child)
    | literalNode(Symbol ltype)
    | literalNode(list[LiteralNodeInfo] names)
    | tupleNode(list[PatternTree] children)
    | typedNameNode(RName name, loc vAt, Symbol vtype, int nameId)
    ...
    | concreteSyntaxNode(Symbol ntype, list[PatternTree] args)
    | varBecomesNode(RName name, loc vAt, PatternTree child, int nameId)
    | asTypeNode(Symbol atype, PatternTree child)
    | deepNode(PatternTree child)
    | antiNode(PatternTree child)
    | tvarBecomesNode(Symbol vtype, RName name, loc vAt, PatternTree child, int nameId)
    ;
			...
			while(modified) {
				...
				for(t <- aliases) {
					...
					if(unsetRec(aliasedType) != unsetRec(ci.store[aliasId].rtype) 
					   && aliasedType.at != ci.store[aliasId].rtype.at) {
						modified = true;
					}
				}
			}
