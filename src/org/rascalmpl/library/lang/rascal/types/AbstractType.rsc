...
public Symbol lub(Symbol s, Symbol t) = lub(unset(s), unset(t))  when getAnnotations(s) != () || getAnnotations(t) != ();

// TODO: rewrite next two rules with keyword parameters and without when condition later:
public bool subtype(Symbol s, Symbol t) = subtype(unset(s), unset(t)) when getAnnotations(s) != () || getAnnotations(t) != ();
public bool subtype(Symbol::user(RName r,  list[Symbol] ps), Symbol s) = subtype(user(unset(r), ps), s) when getAnnotations(r) != ();
public bool subtype(Symbol t, user(RName r, list[Symbol] ps)) = subtype(t, user(unset(r), ps)) when getAnnotations(r) != ();
