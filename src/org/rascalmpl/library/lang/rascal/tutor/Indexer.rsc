...
rel[str, str] createConceptIndex(loc src)
  ...
  + // Here come the index entries for Rascal modules and declarations:
    {  // `((getDefaultPathConfig))` -> `#util-Reflective-getDefaultPathConfig`
      *{<"<item.kind>:<item.name>","/<capitalize(src.file)>/<moduleFragment(item.moduleName)>/#<item.name>">, <item.name, "/<capitalize(src.file)>/<moduleFragment(item.moduleName)>.md#<item.moduleName>-<item.name>" > | item.name?},
