...
module lang::rascal::iguana::ParserGenerator
...
rel[int,int] computeDontNests(Items items, Grammar iguana) {
  // first we compute a map from productions to their last items (which identify each production)
  prodItems = (p:items[getType(rhs)][item(p,size(lhs)-1)].itemId | /Production p:prod(Symbol rhs,list[Symbol] lhs, _) := iguana);
  ...
  return {<items[getType(father.def)][item(father,pos)].itemId, prodItems[child]> | <father,pos,child> <- dnn, father is prod}
       + {<getItemId(t, pos, child), prodItems[child]> | <regular(s),pos,child> <- dnn, /Symbol t := iguana, s == t};
}
...
@doc{This function generates Java code to allocate a new item for each position in the iguana.
...
public str generateSeparatorExpects(Grammar iguana, list[Symbol] seps) {
   if (seps == []) {
     return "";
   }
   
   return (sym2newitem(iguana, head(seps), 1).new | it + ", <sym2newitem(iguana, seps[i+1], i+2).new>" | int i <- index(tail(seps)));
}

public str generateSequenceExpects(Grammar iguana, list[Symbol] seps) {
   if (seps == []) {
     return "";
   }
   
   return (sym2newitem(iguana, head(seps), 0).new | it + ", <sym2newitem(iguana, seps[i+1], i+1).new>" | int i <- index(tail(seps)));
}

public str generateAltExpects(Grammar iguana, list[Symbol] seps) {
   if (seps == []) {
     return "";
   }
   
   return (sym2newitem(iguana, head(seps), 0).new | it + ", <sym2newitem(iguana, seps[i+1], 0).new>" | int i <- index(tail(seps)));
}
...
public tuple[str new, int itemId] sym2newitem(Grammar iguana, Symbol sym, int dot){
    ...
    if (conditional(def, conds) := sym) {
      conds = expandKeywords(iguana, conds);
    ...
    switch (sym) {
        ...
        case \start(s) : 
            return <"new NonTerminalStackNode\<IConstructor\>(<itemId>, <dot>, \"<sym2name(sym)>\", <filters>)", itemId>;
        case \lit(l) : 
            if (/p:prod(sym,list[Symbol] chars,_) := iguana.rules[sym])
                return <"new LiteralStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(p)>, new int[] {<literals2ints(chars)>}, <filters>)",itemId>;
            else throw "literal not found in iguana: <iguana>";
        case \cilit(l) : 
            if (/p:prod(sym,list[Symbol] chars,_) := iguana.rules[sym])
                return <"new CaseInsensitiveLiteralStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(p)>, new int[] {<literals2ints(chars)>}, <filters>)",itemId>;
            else throw "ci-literal not found in iguana: <iguana>";
        case \iter(s) : 
            return <"new ListStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(regular(sym))>, <sym2newitem(iguana, s,  0).new>, true, <filters>)",itemId>;
        case \iter-star(s) :
            return <"new ListStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(regular(sym))>, <sym2newitem(iguana, s,  0).new>, false, <filters>)", itemId>;
        case \iter-seps(Symbol s,list[Symbol] seps) : {
            reg = regular(sym);
            return <"new SeparatedListStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(reg)>, <sym2newitem(iguana, s,  0).new>, (AbstractStackNode\<IConstructor\>[]) new AbstractStackNode[]{<generateSeparatorExpects(iguana,seps)>}, true, <filters>)",itemId>;
        }
        case \iter-star-seps(Symbol s,list[Symbol] seps) : {
            reg = regular(sym);
            return <"new SeparatedListStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(reg)>, <sym2newitem(iguana, s,  0).new>, (AbstractStackNode\<IConstructor\>[]) new AbstractStackNode[]{<generateSeparatorExpects(iguana,seps)>}, false, <filters>)",itemId>;
        }
        case \opt(s) : {
            reg =  regular(sym);
            return <"new OptionalStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(reg)>, <sym2newitem(iguana, s,  0).new>, <filters>)", itemId>;
        }
        case \alt(as) : {
            alts = [a | a <- as];
            return <"new AlternativeStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(regular(sym))>, (AbstractStackNode\<IConstructor\>[]) new AbstractStackNode[]{<generateAltExpects(iguana,  alts)>}, <filters>)", itemId>;
        }
        case \seq(ss) : {
            return <"new SequenceStackNode\<IConstructor\>(<itemId>, <dot>, <value2id(regular(sym))>, (AbstractStackNode\<IConstructor\>[]) new AbstractStackNode[]{<generateSequenceExpects(iguana,  ss)>}, <filters>)", itemId>;
        }
