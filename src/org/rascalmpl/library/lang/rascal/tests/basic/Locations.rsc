...
test bool pathAdditions(list[str] ss) = (|tmp:///ba| | it + fixPathAddition(s)  | s <- ss, s != "" ).path == ("/ba" | it + "/" + fixPathAddition(s)  | s <- ss, s != "" );
test bool pathAdditions(loc l, str s) = s == "" || (l + fixPathAddition(s)).path == ((endsWith(l.path, "/") ? l.path : l.path + "/") + fixPathAddition(s)) ;
