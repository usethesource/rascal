...
public class TutorCommandExecutor {
    ...
    public TutorCommandExecutor(PathConfig pcfg) throws IOException, URISyntaxException {
        ...
        repl = new RascalInterpreterREPL(false, false, null) {
            @Override
            protected Evaluator constructEvaluator(InputStream input,
                                                   OutputStream stdout,
                                                   OutputStream stderr,
                                                   IDEServices services) {
                Evaluator eval = ShellEvaluatorFactory.getDefaultEvaluator(input, stdout, stderr);
                eval.getConfiguration()
                    .setRascalJavaClassPathProperty(javaCompilerPathAsString(pcfg.getJavaCompilerPath()));
            ...
            for (String mimeType : output.keySet()) {
                ...
                if (mimeType.equals("text/plain")) {
                    ...
                    shellHTMLOutput.write(
                        Prelude.consumeInputStream(new InputStreamReader(content, StandardCharsets.UTF_8)).getBytes());
