...
data Declaration
    ...
    | \enum(Expression name, list[Type] implements, list[Declaration] constants, list[Declaration] body)
    | \enumConstant(Expression name, list[Expression] arguments, Declaration class)
    | \enumConstant(Expression name, list[Expression] arguments)
    | \class(Expression name, list[Type] extends, list[Type] implements, list[Declaration] body)
    | \class(list[Declaration] body)
    | \interface(Expression name, list[Type] extends, list[Type] implements, list[Declaration] body)
    | \field(Type \type, list[Expression] fragments)
    | \initializer(Statement initializerBody)
    | \method(Type \return, Expression name, list[Declaration] parameters, list[Expression] exceptions, Statement impl)
    | \method(Type \return, Expression name, list[Declaration] parameters, list[Expression] exceptions)
    | \constructor(Expression name, list[Declaration] parameters, list[Expression] exceptions, Statement impl)
    | \import(Expression name)
    | \package(Expression name)
    ...
    | \annotationType(Expression name, list[Declaration] body)
    | \annotationTypeMember(Type \type, Expression name)
    | \annotationTypeMember(Type \type, Expression name, Expression defaultBlock)
    // initializers missing in parameter, is it needed in vararg?
    | \parameter(Type \type, Expression name, int extraDimensions)
    | \vararg(Type \type, Expression name)
    ;
...
data Expression(TypeSymbol typ=\unresolved())
    ...
    | \memberValuePair(str identifier, Expression \value)
