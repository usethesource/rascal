...
data Expression(TypeSymbol typ=\unresolved())
    ...
    | \fieldAccess(bool isSuper, Expression expression, str identifier)
    | \fieldAccess(bool isSuper, str identifier)
    | \instanceof(Expression leftSide, Type rightSide)
    | \methodCall(bool isSuper, str identifier, list[Expression] arguments)
    | \methodCall(bool isSuper, Expression receiver, str identifier, list[Expression] arguments)
    | \null()
    ...
    | \type(Type \type)
    | \variable(str identifier, int extraDimensions)
    | \variable(str identifier, int extraDimensions, Expression \initializer)
