...
public M3 createM3FromDirectory(loc project, bool errorRecovery = false, str javaVersion = "1.7") {
    ...
    m3s = composeJavaM3(project, createM3sFromFiles({p | sp <- sourcePaths, p <- find(sp, "java"), isFile(p)}, errorRecovery = errorRecovery, sourcePath = findRoots([*sourcePaths]), classPath = classPaths, javaVersion = javaVersion));
