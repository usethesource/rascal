...
public alias I = int;
public alias S = tuple[I,str];
public alias S1 = tuple[I,lrel[str,str]];
public alias S2 = tuple[I,tuple[str,str]];
...
public S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), pp([ y[1] | S y <- ys ], "")>;
...
public Attr[I,S] toC1(FModule[Attr[I,S],Attr[I,S],Attr[I,S]] n, type[Module] \type = #Module) { 
	...
	I g(I x, S ys...) = attrFunc([ y[0] | S y <- ys ]); // note: it introduces circularity
...
public Attr[I,S] toC3(FConstDecl[Attr[I,S],Attr[I,S]] n, type[ConstDecl] \type = #ConstDecl) {
	...
	S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), "#define " + at(ys,0)[1] + " " + at(ys,1)[1] + ";\n">;
...
public Attr[I,S] toC4(FTypeDecl[Attr[I,S],Attr[I,S]] n, type[TypeDecl] \type = #TypeDecl) {
	...
	S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), "typedef " + at(ys,0)[1] + " " + at(ys,1)[1] + ";\n">;
...
public Attr[I,S] toC5(FVarDecl[Attr[I,S],Attr[I,S]] n, type[VarDecl] \type = #VarDecl) { 
	...
	S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), at(ys,0)[1] + " " + at(ys,1)[1] + ";\n">;
...
public Attr[I,S] toC7(FStatement[Attr[I,S],Attr[I,S],Attr[I,S],Attr[I,S1]] n, type[Statement] \type = #Statement) {
	switch(n) {
		...
		case ifThen(Attr[I,S] condition, Attr[I,S] body, Attr[I,S1] elseIfs, Attr[I,S] elsePart) : {
			Attr[I,S] elseIfs_ = attr(S (I x) { S1 telseIfs = elseIfs.f(x); return <telseIfs[0], "<for(tuple[str,str] elseIf <- telseIfs[1]){> else if (<elseIf[0]>) { <elseIf[1]> }<}>">; });
			S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), "if (<at(ys,0)[1]>) { <at(ys,1)[1]> } <at(ys,2)[1]> <if(at(ys,3)[1] != "") {> else { <at(ys,3)[1]> }<}>" + ";\n">;
			return evalAttr(f, [ condition, body, elseIfs_, elsePart ], [ g, g, g, g ], false);
		}
		case whileDo(Attr[I,S] condition, Attr[I,S] body) : {
			S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), "while(<at(ys,0)[1]>) { <at(ys,1)[1]> }" + ";\n">;
...
public Attr[I,S] toC8(FExpression[Attr[I,S],Attr[I,S],Attr[I,S]] n, type[Expression] \type = #Expression) {
	switch(n) {
		...
		case neg(Attr[I,S] exp): { 
			S f(I x, S ys...) = <attrFunc([ y[0] | S y <- ys ]), "(-<at(ys,0)[1]>)">; 
...
public Attr[I,S] toC12(lrel[Attr[I,S],Attr[I,S]] n, type[list[TypeDecl]] \type = #list[TypeDecl]) {
	if([<Attr[I,S] el0, Attr[I,S] rest>] := n)
		return evalAttr(f, [ el0, rest ], [ g, g ], false);
	return attr(tuple[int,str] (int x) { return <0,"">; });
}
...
public Attr[I,S1] toC15(lrel[Attr[I,S2],Attr[I,S1]] n, type[lrel[Expression,list[Statement]]] \type = #lrel[Expression,list[Statement]]) {
	if([<Attr[I,S2] el0, Attr[I,S1] rest>] := n)
			return attr(S1 (I x) { return <attrFunc(el0.f(x/*note:g(x,ys)=x*/)[0] + rest.f(x/*note:g(x,ys)=x*/)[0]), [el0.f(x/*note:g(x,ys)=x*/)[1]] + rest.f(x/*note:g(x,ys)=x*/)[1]>; });
...
public Attr[I,S2] toC16(tuple[Attr[I,S],Attr[I,S]] n, type[tuple[Expression,list[Statement]]] \type = #tuple[Expression,list[Statement]]) {
	...
	return attr(S2 (I x) { return <attrFunc(el1.f(x)[0], el2.f(x)[0]), <el1.f(x)[1], el2.f(x)[1]>>; });
}
...
public str test0(loc l) {
	...
	Attr[int,tuple[int,str]] (Module) f = fvisit[<#Module>, <toC1,toC2,toC3,toC4,toC5,toC6,toC7,toC8,toC9,toC10,toC11,toC12,toC13,toC14,toC15,toC16,toC17,toC18>];
	Attr[int,tuple[int,str]] m010 = f(m);
	return m010.f(0)[1];
}
