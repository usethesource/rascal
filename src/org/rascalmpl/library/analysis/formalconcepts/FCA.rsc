...
public ConceptLattice[&Object, &Attribute] fca(FormalContext[&Object, &Attribute] fc) {
    ...
    return
        {<<tau(fc, c1), c1>, <tau(fc, c2), c2>>
        | <set[&Attribute] c1,set[&Attribute] c2> <- lat
        };
}
...
public FormalContext[&Object, &Attribute] toFormalContext(Object2Attributes[&Object, &Attribute] objects) {
    return
        {<object, attribute>
        | &Object object <- domain(objects), &Attribute attribute <- objects[object]
        };
}
...
public FormalContext[&Object, &Attribute] toFormalContext(Attribute2Objects[&Object, &Attribute] attributes) {
    return
        {<object, attribute>
        | &Attribute attribute <- domain(attributes), &Object object <- attributes[attribute]
        };
}
...
set[set[&T]] maxincl(set[set[&T]] c) {
    return {s| set[&T] s <- c, !isSubset(c, s)};
}
