...
private str viewTree1(Tree t){
  ...
  switch(t){
    case char(int c) : {
        root = newId();
        nodes += box([viz::Figure::Core::id(root), gap(1), fontColor("blue")], text([textAngle(angle)], escape(stringChar(c))));
        return root;
    }
    case str s: {
      root = newId();
    ...
    case appl(Production prod, list[Tree] args):
     if(prod.rhs == \cf(\opt(\layout()))){
        root = newId();
        nodes += ellipse([size(4), viz::Figure::Core::id(root), fillColor("grey"), popup("LAYOUT?")]);
        return root;
     } else if(\layouts(_) := prod.rhs){
        root = newId();
        nodes += ellipse([size(4), viz::Figure::Core::id(root), fillColor("grey"), popup("LAYOUTLIST")]);
        return root;
     } else {
	     FProperty p = popup(viewProduction(prod));
	     root = newId();
	     viewTrees(root, args);
	     nodes += ellipse([viz::Figure::Core::id(root), size(4), p]);
	     return root;
     }
     ...
     case amb(set[Tree] alternatives):{
         ...
         nodes += ellipse([viz::Figure::Core::id(root), size(10), fillColor("red"), p]);
	     return root; 
      }
  }
...
private void viewTrees(str root, list[Tree] trees){
  if(allChars(trees)){
    ...
    nodes += box([viz::Figure::Core::id(this), gap(1), fontColor("blue")], text([textAngle(angle)], chars));
