...
public str toHTML(Tree input, bool withStyle=false) {
  htmlEscapes = (
	  "\<": "&lt;",
	  "\>": "&gt;",
	  "&" : "&amp;"
  ...
  str yield(list[Tree] as) = "<for (a <- as) {><a><}>";
  ...
  if (withStyle) {
    return "\<html\>\<body\>\<head\>
           ...
           '<ToHTMLStyle()>
           '\</style\>
           '\<pre class=\"rascal\"\>\<code\><trim(rec(input))>\</code\>\</pre\>
           '\</body\>\<html\>";

  }
  else {
    return "\<pre class=\"rascal\"\>\<code\><trim(rec(input))>\</code\>\</pre\>";
  }
}
