...
public class ASTBuilder {
	...
	public Module buildModule(org.rascalmpl.core.core.values.uptr.ITree tree) throws FactTypeUseException {
	...
	public Expression buildExpression(org.rascalmpl.core.values.uptr.ITree parseTree) {
		return buildSort(parseTree, "Expression");
	}

	public Statement buildStatement(org.rascalmpl.core.values.uptr.ITree parseTree) {
		return buildSort(parseTree, "Statement");
	}

	public Command buildCommand(org.rascalmpl.core.values.uptr.ITree parseTree) {
		return buildSort(parseTree, "Command");
	}

	public Command buildSym(org.rascalmpl.core.values.uptr.ITree parseTree) {
		return buildSort(parseTree, "Sym");
	}

	public Commands buildCommands(org.rascalmpl.core.values.uptr.ITree parseTree) {
		return buildSort(parseTree, "Commands");
	}

	@SuppressWarnings("unchecked")
	public <T extends AbstractAST> T buildSort(org.rascalmpl.core.values.uptr.ITree parseTree, String sort) {
		if (TreeAdapter.isAppl(parseTree)) {
			org.rascalmpl.core.values.uptr.ITree tree = TreeAdapter.getStartTop(parseTree);
	...
	public AbstractAST buildValue(IValue arg)  {
		org.rascalmpl.core.values.uptr.ITree tree = (org.rascalmpl.core.values.uptr.ITree) arg;
		...
		if (isLexical(tree)) {
			...
			return buildLexicalNode((org.rascalmpl.core.values.uptr.ITree) ((IList) ((org.rascalmpl.core.values.uptr.ITree) arg).get("args")).get(0));
		}
		...
		return buildContextFreeNode((org.rascalmpl.core.values.uptr.ITree) arg);
	}

	private List<AbstractAST> buildList(org.rascalmpl.core.values.uptr.ITree in)  {
	...
	private AbstractAST buildContextFreeNode(org.rascalmpl.core.values.uptr.ITree tree)  {
		...
		int i = 2;
		for (IValue arg : args) {
			org.rascalmpl.core.values.uptr.ITree argTree = (org.rascalmpl.core.values.uptr.ITree) arg;

			if (TreeAdapter.isList(argTree)) {
				actuals[i] = buildList((org.rascalmpl.core.values.uptr.ITree) arg);
			}
			else {
	...
	private AbstractAST buildLexicalNode(org.rascalmpl.core.values.uptr.ITree tree) {
	...
	private String getPatternLayout(org.rascalmpl.core.values.uptr.ITree tree) {
	...
	private Expression liftRec(org.rascalmpl.core.values.uptr.ITree tree, boolean lexicalFather, String layoutOfFather) {
		...
		if (TreeAdapter.isAppl(tree)) {
			...
			for (IValue arg : args) {
				Expression ast = liftRec((org.rascalmpl.core.values.uptr.ITree) arg, lex, layout);
				if (ast == null) {
					return null;
				}
				kids.add(ast);
			...
			for (IValue arg : args) {
				kids.add(liftRec((org.rascalmpl.core.values.uptr.ITree) arg, lexicalFather, layoutOfFather));
			}
	...
	private Expression liftHole(org.rascalmpl.core.values.uptr.ITree tree) {
		assert tree.asAnnotatable().hasAnnotation("holeType");
		IConstructor type = (IConstructor) tree.asAnnotatable().getAnnotation("holeType");
		tree = (org.rascalmpl.core.values.uptr.ITree) TreeAdapter.getArgs(tree).get(0);
		IList args = TreeAdapter.getArgs(tree);
	...
	private IList getASTArgs(org.rascalmpl.core.values.uptr.ITree tree) {
		IList children = TreeAdapter.getArgs(tree);
		IListWriter writer = ValueFactoryFactory.getValueFactory().listWriter();
		
                for (int i = 0; i < children.length(); i++) {
			org.rascalmpl.core.values.uptr.ITree kid = (org.rascalmpl.core.values.uptr.ITree) children.get(i);
	...
	private String sortName(org.rascalmpl.core.values.uptr.ITree tree) {
		if (TreeAdapter.isAppl(tree)) { 
			return TreeAdapter.getSortName(tree);
		}
		if (TreeAdapter.isAmb(tree)) {
			// all alternatives in an amb cluster have the same sort
			return sortName((org.rascalmpl.core.values.uptr.ITree) TreeAdapter.getAlternatives(tree).iterator().next());
		}
		return "";
	}
	...
	private boolean isNewEmbedding(org.rascalmpl.core.values.uptr.ITree tree) {
	...
	private boolean isLexical(org.rascalmpl.core.values.uptr.ITree tree) {
		if (TreeAdapter.isRascalLexical(tree)) {
			return true;
		}
		return false;
	...
	private AbstractAST newLift(org.rascalmpl.core.values.uptr.ITree tree, boolean match) {
		org.rascalmpl.core.values.uptr.ITree concrete = (org.rascalmpl.core.values.uptr.ITree) TreeAdapter.getArgs(tree).get(0);
		org.rascalmpl.core.values.uptr.ITree fragment = (org.rascalmpl.core.values.uptr.ITree) TreeAdapter.getArgs(concrete).get(7);
	...
	private static AbstractAST callMakerMethod(String sort, String cons, Object actuals[], Object keywordActuals[]) {
		try {
			...
			if (constructor == null) {
				...
				try {
					clazz = classLoader.loadClass("org.rascalmpl.core.semantics.dynamic." + name);
				}
