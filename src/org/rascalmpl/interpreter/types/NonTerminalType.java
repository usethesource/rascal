...
public class NonTerminalType extends RascalType {
	...
	@Override
	public boolean match(Type other, Map<Type, Type> bindings)
		...
		else if (SymbolAdapter.isSeq(symbol) && SymbolAdapter.isSeq(otherSym)) {
			IList elems1 = SymbolAdapter.getSymbols(symbol);
			IList elems2 = SymbolAdapter.getSymbols(otherSym);
			if (elems1.length() != elems2.length()) {
				return false;
			}
			
			for (int i = 0; i < elems1.length(); i++) {
				RascalType nt1 = (RascalType) RTF.nonTerminalType((IConstructor) elems1.get(i));
				RascalType nt2 = (RascalType) RTF.nonTerminalType((IConstructor) elems1.get(i));
				if (!nt1.match(nt2, bindings)) {
					return false;
				}
			}
		...
		else if (SymbolAdapter.isAlt(symbol) && SymbolAdapter.isAlt(otherSym)) {
			ISet alts1 = SymbolAdapter.getAlternatives(symbol);
			ISet alts2 = SymbolAdapter.getAlternatives(otherSym);
			
			// for all elements in alts1 there must be at least one supertype in alts2
			outer:for (IValue alt1 : alts1) {
				RascalType nt1 = (RascalType) RTF.nonTerminalType((IConstructor) alt1);
				
				for (IValue alt2 : alts2) {
					RascalType nt2 = (RascalType) RTF.nonTerminalType((IConstructor) alt2);
				...
				// no supertype found for this alt1
				return false;
			}
