...
public class ConcreteSyntaxResult extends ConstructorResult {
	...
	public <U extends IValue> Result<U> fieldAccess(String name, TypeStore store) {
		...
		Type kwType = store.getKeywordParameterType(RascalValueFactory.Tree_Appl, name);
		
		if (kwType != null) {
			IValue parameter = tree.asWithKeywordParameters().getParameter(name);
		
			if (parameter == null) {  
				ConstructorFunction appl = ctx.getCurrentEnvt().getConstructorFunction(RascalValueFactory.Tree_Appl);
