...
public class SourceLocationResult extends ElementResult<ISourceLocation> {
	...
	public <U extends IValue> Result<U> fieldAccess(String name, TypeStore store) {
		...
		case "ls": {
			try {
				if (!URIResolverRegistry.getInstance().isDirectory(value.getURI())) {
				...
				Result<IValue> resRes = makeResult(getType(), value, ctx);

				IListWriter w = ctx.getValueFactory().listWriter();
				Type stringType = tf.stringType();

				for (String elem : URIResolverRegistry.getInstance().listEntries(value.getURI())) {
