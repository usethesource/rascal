...
/* "fun" type:Type Parameters "{" statements:Statement* "}" -> Expression {cons("Closure")} */
	private Closure() { }
	/*package*/ Closure(ITree tree, Type type, List<Statement> statements) {
		this.tree = tree;
		this.type = type;
...
/* "(" closure:Expression ")" "(" arguments:{Expression ","}* ")" -> Expression {cons("ClosureCall")} */
	private ClosureCall() { }
	/*package*/ ClosureCall(ITree tree, Expression closure, List<Expression> arguments) {
		this.tree = tree;
...
private Expression closure;
	public Expression getclosure() { return closure; }
	private void privateSetclosure(Expression x) { this.closure = x; }
	...
	private void privateSetarguments(List<Expression> x) { this.arguments = x; }
...
/* expression:Expression "[" key:Name "->" replacement:Expression "]" -> Expression {cons("FieldUpdate")} */
	private FieldUpdate() { }
	/*package*/ FieldUpdate(ITree tree, Expression expression, Name key, Expression replacement) {
		this.tree = tree;
	...
	public IVisitable accept(IASTVisitor visitor) {
		return visitor.visitFieldUpdateExpression(this);
	}
private Expression expression;
	...
	private void privateSetkey(Name x) { this.key = x; }
...
/* expression:Expression "[" subscript:Expression "]" -> Expression {cons("Subscript")} */
	private Subscript() { }
	/*package*/ Subscript(ITree tree, Expression expression, Expression subscript) {
		this.tree = tree;
		this.expression = expression;
...
private Expression argument;
	...
	private void privateSetargument(Expression x) { this.argument = x; }
...
private Expression lhs;
	...
	private void privateSetlhs(Expression x) { this.lhs = x; }
	...
	private void privateSetrhs(Expression x) { this.rhs = x; }
...
/* condition:Expression "?" then:Expression ":" else:Expression -> Expression {right, cons("IfThenElse")} */
	private IfThenElse() { }
	/*package*/ IfThenElse(ITree tree, Expression condition, Expression then, Expression else) {
		this.tree = tree;
...
private Expression condition;
	...
	public Expression getthen() { return then; }
	private void privateSetthen(Expression x) { this.then = x; }
	...
	public Expression getelse() { return else; }
	private void privateSetelse(Expression x) { this.else = x; }
...
private Literal literal;
	...
	private void privateSetliteral(Literal x) { this.literal = x; }
...
/* name:Name "(" arguments:{Expression ","}* ")" -> Expression {cons("CallOrTree")} */
	private CallOrTree() { }
	/*package*/ CallOrTree(ITree tree, Name name, List<Expression> arguments) {
		this.tree = tree;
		this.name = name;
		this.arguments = arguments;
	}
...
private List<Expression> elements;
	...
	private void privateSetelements(List<Expression> x) { this.elements = x; }
...
private Expression from;
	public Expression getfrom() { return from; }
	private void privateSetfrom(Expression x) { this.from = x; }
	...
	public Expression getto() { return to; }
	private void privateSetto(Expression x) { this.to = x; }
...
/* "[" from:Expression "," by:Expression ",.." to:Expression "]" -> Expression {cons("StepRange")} */
	private StepRange() { }
	/*package*/ StepRange(ITree tree, Expression from, Expression by, Expression to) {
		this.tree = tree;
		this.from = from;
		this.by = by;
		this.to = to;
...
private Expression from;
	...
	public Expression getby() { return by; }
	private void privateSetby(Expression x) { this.by = x; }
...
/* "<" first:Expression "," rest:{Expression ","}+ ">" -> Expression {cons("Tuple")} */
	private Tuple() { }
	/*package*/ Tuple(ITree tree, Expression first, List<Expression> rest) {
		this.tree = tree;
		this.first = first;
		this.rest = rest;
	}
...
private Expression first;
	...
	private void privateSetfirst(Expression x) { this.first = x; }
	...
	public List<Expression> getrest() { return rest; }
	private void privateSetrest(List<Expression> x) { this.rest = x; }
...
private Expression filename;
	public Expression getfilename() { return filename; }
	private void privateSetfilename(Expression x) { this.filename = x; }
...
private Expression area;
	public Expression getarea() { return area; }
	private void privateSetarea(Expression x) { this.area = x; }
...
/* "area-in-file" "(" filename:Expression "," area:Expression ")" -> Expression {cons("AreaInFileLocation")} */
	...
	/*package*/ AreaInFileLocation(ITree tree, Expression filename, Expression area) {
		this.tree = tree;
		this.filename = filename;
		this.area = area;
	}
...
private Comprehension comprehension;
	public Comprehension getcomprehension() { return comprehension; }
	private void privateSetcomprehension(Comprehension x) { this.comprehension = x; }
...
/* "exists" "(" producer:ValueProducer "|" expression:Expression ")" -> Expression {cons("Exists")} */
	private Exists() { }
	/*package*/ Exists(ITree tree, ValueProducer producer, Expression expression) {
		this.tree = tree;
...
private ValueProducer producer;
	public ValueProducer getproducer() { return producer; }
	private void privateSetproducer(ValueProducer x) { this.producer = x; }
...
/* "forall" "(" producers:ValueProducer "|" expression:Expression ")" -> Expression {cons("Forall")} */
	private Forall() { }
	/*package*/ Forall(ITree tree, ValueProducer producers, Expression expression) {
		this.tree = tree;
...
private ValueProducer producers;
	public ValueProducer getproducers() { return producers; }
	private void privateSetproducers(ValueProducer x) { this.producers = x; }
...
private Visit visit;
	...
	private void privateSetvisit(Visit x) { this.visit = x; }
