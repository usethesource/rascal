...
/* character:[a-zA-Z0-9] -> ShortChar {cons("Regular")} */
    ...
    /*package */ Regular (ITree tree,
			  List <
			  get - sort - from - symbol ([a - zA - Z0 - 9]) >
			  character)
    {
      this.tree = tree;
    ...
    {
      return visitor.visitShortCharRegular (this);
    }
    ...
    private void privateSetcharacter (List <
				      get - sort - from -
				      symbol ([a - zA - Z0 - 9]) > x)
    {
      this.character = x;
    }
    public Regular setcharacter (List <
				 get - sort - from -
				 symbol ([a - zA - Z0 - 9]) > x)
    {
...
/* [\\] escape:~[\0-\31A-Za-mo-qsu-z0-9] -> ShortChar {cons("Escaped")} */
    ...
    /*package */ Escaped (ITree tree,
			  List <
			  get - sort - from -
			  symbol (~[\0 - \31 A - Za - mo - qsu - z0 - 9]) >
			  escape)
    {
      this.tree = tree;
      this.escape = escape;
    ...
    public IVisitable accept (IASTVisitor visitor)
    {
      return visitor.visitShortCharEscaped (this);
    }
    private List <
      get - sort - from - symbol (~[\0 - \31 A - Za - mo - qsu - z0 - 9]) >
      escape;
    public List <
      get - sort - from - symbol (~[\0 - \31 A - Za - mo - qsu - z0 - 9]) >
      getescape ()
    ...
    private void privateSetescape (List <
				   get - sort - from -
				   symbol (~
					   [\0 - \31 A - Za - mo - qsu - z0 -
					    9]) > x)
