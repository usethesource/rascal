...
/* "solve" bound:Bound init:Statement "do" body:Statement -> Statement {cons("Solve")} */
	private Solve() { }
	/*package*/ Solve(ITree tree, Bound bound, Statement init, Statement body) {
		this.tree = tree;
		this.bound = bound;
		this.init = init;
		this.body = body;
...
private Bound bound;
	...
	private void privateSetbound(Bound x) { this.bound = x; }
	...
	private void privateSetinit(Statement x) { this.init = x; }
	...
	private void privateSetbody(Statement x) { this.body = x; }
...
/* "for" "(" generators:{Generator ","}+ ")" body:Statement -> Statement {cons("For")} */
	private For() { }
	/*package*/ For(ITree tree, List<Generator> generators, Statement body) {
		this.tree = tree;
...
private List<Generator> generators;
	...
	private void privateSetgenerators(List<Generator> x) { this.generators = x; }
...
/* "if" "(" condition:Condition ")" thenStatement:Statement "else" elseStatement:Statement -> Statement {cons("IfThenElse")} */
	private IfThenElse() { }
	/*package*/ IfThenElse(ITree tree, Condition condition, Statement thenStatement, Statement elseStatement) {
		this.tree = tree;
		this.condition = condition;
...
private Condition condition;
	...
	private void privateSetcondition(Condition x) { this.condition = x; }
	...
	private void privateSetthenStatement(Statement x) { this.thenStatement = x; }
	...
	private void privateSetelseStatement(Statement x) { this.elseStatement = x; }
...
/* "if" "(" condition:Condition ")" thenStatement:Statement NoElseMayFollow -> Statement {cons("IfThen")} */
	private IfThen() { }
	/*package*/ IfThen(ITree tree, Condition condition, Statement thenStatement) {
		this.tree = tree;
		this.condition = condition;
		this.thenStatement = thenStatement;
	}
...
/* "switch" "(" expression:Expression ")" "{" cases:Case+ "}" -> Statement {cons("Switch")} */
	private Switch() { }
	/*package*/ Switch(ITree tree, Expression expression, List<Case> cases) {
		this.tree = tree;
...
private Expression expression;
	...
	private void privateSetcases(List<Case> x) { this.cases = x; }
...
/* assignables:{Assignable ","}+ operator:Assignment expressions:{Expression ","}+ ";" -> Statement {cons("Assignment")} */
	private Assignment() { }
	/*package*/ Assignment(ITree tree, List<Assignable> assignables, Assignment operator, List<Expression> expressions) {
		this.tree = tree;
...
private List<Assignable> assignables;
	public List<Assignable> getassignables() { return assignables; }
	private void privateSetassignables(List<Assignable> x) { this.assignables = x; }
	...
	public Assignment getoperator() { return operator; }
	private void privateSetoperator(Assignment x) { this.operator = x; }
	...
	private void privateSetexpressions(List<Expression> x) { this.expressions = x; }
...
/* "assert" label:StringLiteral ":" expression:Expression ";" -> Statement {cons("Assert")} */
	private Assert() { }
	/*package*/ Assert(ITree tree, StringLiteral label, Expression expression) {
		this.tree = tree;
		this.label = label;
		this.expression = expression;
	}
...
private StringLiteral label;
	public StringLiteral getlabel() { return label; }
	private void privateSetlabel(StringLiteral x) { this.label = x; }
...
private Statement body;
	...
	public List<Catch> gethandlers() { return handlers; }
	private void privateSethandlers(List<Catch> x) { this.handlers = x; }
...
/* "try" body:Statement handlers:Catch+ "finally" finallyBody:Statement -> Statement {cons("TryFinally")} */
	private TryFinally() { }
	/*package*/ TryFinally(ITree tree, Statement body, List<Catch> handlers, Statement finallyBody) {
		this.tree = tree;
		this.body = body;
		this.handlers = handlers;
...
private Statement body;
	...
	private void privateSetfinallyBody(Statement x) { this.finallyBody = x; }
...
/* functionDeclaration:FunctionDeclaration -> Statement {cons("FunctionDeclaration")} */
	...
	/*package*/ FunctionDeclaration(ITree tree, FunctionDeclaration functionDeclaration) {
		this.tree = tree;
...
/* declaration:LocalVariableDeclaration ";" -> Statement {cons("VariableDeclaration")} */
	...
	/*package*/ VariableDeclaration(ITree tree, LocalVariableDeclaration declaration) {
		this.tree = tree;
...
private LocalVariableDeclaration declaration;
	public LocalVariableDeclaration getdeclaration() { return declaration; }
	private void privateSetdeclaration(LocalVariableDeclaration x) { this.declaration = x; }
...
/* "global" type:Type names:{QualifiedName ","}+ ";" -> Statement {cons("GlobalDirective")} */
	...
	/*package*/ GlobalDirective(ITree tree, Type type, List<QualifiedName> names) {
		this.tree = tree;
		this.type = type;
		this.names = names;
	}
	public IVisitable accept(IASTVisitor visitor) {
		return visitor.visitGlobalDirectiveStatement(this);
	}
private Type type;
	...
	public List<QualifiedName> getnames() { return names; }
	private void privateSetnames(List<QualifiedName> x) { this.names = x; }
