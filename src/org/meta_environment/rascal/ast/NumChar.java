...
/* [\\] number:[0-9]+ -> NumChar {cons("Digits")} */
	private Digits() { }
	/*package*/ Digits(ITree tree, List<get-sort-from-symbol([0-9]+)> number) {
		this.tree = tree;
		this.number = number;
	}
	public IVisitable accept(IASTVisitor visitor) {
		return visitor.visitDigitsNumChar(this);
	}
private List<get-sort-from-symbol([0-9]+)> number;
	public List<get-sort-from-symbol([0-9]+)> getnumber() { return number; }
	private void privateSetnumber(List<get-sort-from-symbol([0-9]+)> x) { this.number = x; }
	public Digits setnumber(List<get-sort-from-symbol([0-9]+)> x) { 
		Digits z = new Digits();
