...
public class VisitableRelationNode extends AbstractVisitable {
	...
	public VisitableRelationNode(IRelation root, IConstructor value, List<Visitable> children) {
	...
	public static VisitableRelationNode makeVisitableRelationNode(IRelation relation, IConstructor node) {
		...
		if (adjacencies.get(node) != null) {
			for (IValue s: adjacencies.get(node)) {
				successors.add(makeVisitableRelationNode(relation, (IConstructor)s));
			}
		}
