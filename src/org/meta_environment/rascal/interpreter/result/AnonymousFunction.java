...
public class AnonymousFunction extends AbstractFunction {
	private final List<Statement> body;
	private final boolean isVoidFunction;

	public AnonymousFunction(AbstractAST ast, Evaluator eval, FunctionType functionType,
	...
	public Result<?> call(Type[] actualTypes, IValue[] actuals, IEvaluatorContext ctx) {
		...
		try {
			ctx.setCurrentEnvt(new Environment(declarationEnvironment, ctx.getCurrentEnvt(), ctx.getCurrentAST().getLocation(), ast.getLocation(), "anonymous function"));
			ctx.pushEnv();
